...
p = inputParser;
    ...
    addParameter(p, 'AnnoRange', qq(2024,4):4:qq(2025,4));
    addParameter(p, 'AnnotationXAdjustment', 0.015);
    addParameter(p, 'AnnotationYAdjustment', 0);
    addParameter(p, 'TabRange', {}); 
...
%         figure('Position', [1 42.0182 1117.1 776.73]);
        ...
        if strcmp(params.PlotList{i},'D4L_CPI')
         ...
         % Anotaciones para corrimiento actual
            Sim.scripts.die_anotaciones( ...
                [dat2dec(params.AnnoRange)',dat2dec(params.AnnoRange)'], ...
                [FAN{params.AnnoRange(1)}.data(2), FAN{params.AnnoRange(2)}.data(2),...
                 FAN{params.AnnoRange(1)}.data(end), FAN{params.AnnoRange(2)}.data(end)], ...
                {string(num2str(FAN{params.AnnoRange(1)}.data(2), '%0.2f')),...
                 string(num2str(FAN{params.AnnoRange(2)}.data(2), '%0.2f')),...
                 string(num2str(FAN{params.AnnoRange(1)}.data(end), '%0.2f')),...
                 string(num2str(FAN{params.AnnoRange(2)}.data(end), '%0.2f')),...
                 },...
                'Container', plot_p, ...
                'Color', 'k', ...
        ...
        data_table = [];
        data_table(:, 1) = FAN{params.TabRange}.data(:,end)'; % Percentil 90
        data_table(:, 2) = MODEL.F_pred.(params.PlotList{i}){params.TabRange}; % Mediana
        data_table(:, 3) = FAN{params.TabRange}.data(:,2)'; % Percentil 10
        text_Color = ['k' ; 'k'; 'k'];
        ...
        Sim.scripts.plot_data_table( ...
            ...
            'Parent', table_p, ...
            'SeriesNames',...
                {sprintf('Percentil %d', grilla(1)*100),...
                'Mediana',...
                sprintf('Percentil %d', grilla(end)*100)}, ...
        ...
        if  isempty(params.Esc)
            MODEL.Esc.v0.Fanchart.(params.PlotList{i}).FAN =  FAN;
            ...
            MODEL.Esc.(params.Esc).Fanchart.(params.PlotList{i}).FAN =  FAN;
